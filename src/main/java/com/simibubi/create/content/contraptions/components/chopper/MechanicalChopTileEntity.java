...
public class MechanicalChopTileEntity extends BasinOperatingTileEntity {

	private static final Object choppingRecipesKey = new Object();
	public List<ItemStack> choppedItems = new ArrayList<>();
	...
	public MechanicalChopTileEntity(TileEntityType<? extends MechanicalChopTileEntity> type) {
		super(type);
		mode = Mode.WORLD;
	...
	public void addBehaviours(List<TileEntityBehaviour> behaviours) {
		...
		processingBehaviour =
			new BeltProcessingBehaviour(this).whenItemEnters((s, i) -> BeltChopCallbacks.onItemReceived(s, i, this))
				.whileItemHeld((s, i) -> BeltChopCallbacks.whenItemHeld(s, i, this));
	...
	protected void fromTag(BlockState state, CompoundNBT compound, boolean clientPacket) {
		...
		if (clientPacket) {
			NBTHelper.iterateCompoundList(compound.getList("ParticleItems", NBT.TAG_COMPOUND),
				c -> choppedItems.add(ItemStack.read(c)));
			spawnParticles();
		}
	}
	...
	public void write(CompoundNBT compound, boolean clientPacket) {
		...
		if (clientPacket) {
			compound.put("ParticleItems", NBTHelper.writeCompoundList(choppedItems, ItemStack::serializeNBT));
			choppedItems.clear();
		}
	}
			...
			if (hasWorld() && !world.isRemote) {
				...
				if (entityScanCooldown <= 0) {
					...
					for (ItemEntity itemEntity : world.getEntitiesWithinAABB(ItemEntity.class,
						...
						Optional<ChoppingRecipe> recipe = getRecipe(stack);
						if (!recipe.isPresent())
							continue;
						start(Mode.WORLD);
						return;
		...
		if (basin.isPresent()) {
			for (int slot = 0; slot < inputs.getSlots(); slot++) {
				...
				if (stackInSlot.isEmpty())
					continue;
				choppedItems.add(stackInSlot);
			}
		}
		sendData();
	}

	protected void applyChoppingInWorld() {
		...
		for (Entity entity : world.getEntitiesWithinAABBExcludingEntity(null, bb)) {
			...
			choppedItems.add(itemEntity.getItem());
			sendData();
			Optional<ChoppingRecipe> recipe = getRecipe(itemEntity.getItem());
			if (!recipe.isPresent())
				continue;
	...
	protected void spawnParticles() {
		if (choppedItems.isEmpty())
			return;

		if (mode == Mode.BASIN)
			choppedItems.forEach(stack -> makeChoppingWorldParticleEffect(VecHelper.getCenterOf(pos.down(2)), stack));
		if (mode == Mode.BELT)
			choppedItems.forEach(stack -> makeChoppingParticleEffect(VecHelper.getCenterOf(pos.down(2))
				.add(0, 8 / 16f, 0), stack));
		if (mode == Mode.WORLD)
			choppedItems.forEach(stack -> makeChoppingParticleEffect(VecHelper.getCenterOf(pos.down(1))
				.add(0, -1 / 4f, 0), stack));

		choppedItems.clear();
	}

	public void makeChoppingParticleEffect(Vector3d pos, ItemStack stack) {
	...
	public void makeChoppingWorldParticleEffect(Vector3d pos, ItemStack stack) {
		if (world == null || !world.isRemote)
			return;
		for (int i = 0; i < 20; i++) {
	...
	private static final RecipeWrapper choppingInv = new RecipeWrapper(new ItemStackHandler(1));

	public Optional<ChoppingRecipe> getRecipe(ItemStack item) {
		choppingInv.setInventorySlotContents(0, item);
		return AllRecipeTypes.CHOPPING.find(choppingInv, world);
	}

	@Override
	protected <C extends IInventory> boolean matchStaticFilters(IRecipe<C> recipe) {
		return recipe.getType() == AllRecipeTypes.CHOPPING.type;
	}

	@Override
	protected Object getRecipeCacheKey() {
		return choppingRecipesKey;
	}
